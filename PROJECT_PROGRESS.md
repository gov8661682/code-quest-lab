...
- Git state: `main` contains deployed milestone commit `9da1d0e` (`Add local
  ...
  hardening commits between them are also pushed. The deployment record for
  `9da1d0e` is maintained below; these incomplete or minor follow-ups are
  intentionally not deployed as separate checkpoints.
- Canonical game: `index.html`, 57,193 lines, SHA-256
  `302CF0FDB356FFD669FA9211982B82F1E88F243001137BF72C68390545338A19`
...
- Current local verification: `npm.cmd run release:verify` passed 77 tests,
...
- Last website checkpoint: commit `9da1d0e`, deployed and live-verified on
  ...
  surface
- Latest local route evidence: an existing Mage profile completed Dungeon 1
  through the Stone Guardian and exit portal, reaching The Fallen Kingdom
  Entrance with the loopback developer invincibility aid enabled. This is
  partial D1 evidence only; it does not change the clean-profile D1-8 or
  touch-first acceptance status, so the evidence score remains 61% / 16%.
...
| Feature or workstream | Current status | Evidence from the repository | Remaining work | Dependencies | Required for V1? | Recommended next action |
|---|---|---|---|---|---|---|
| Full V1 gameplay and progression | Runtime and automated route coverage exist; player-completed evidence is incomplete | `index.html` contains Town, four classes, combat, equipment, crafting, achievements, dungeon definitions, boss dispatches, and the D1-8 release guard; isolated QA completed Dungeon 1 through its boss/portal, then exposed and fixed a D2 Corrupted-elite summon escalation | Continue the bounded D1-8 path from a clean profile; complete rooms, bosses, final portal, and intended session ending, recording failures and balance issues | Clean browser profile, supported input surface, and stable combat | Yes | Re-test D2 after the summon-budget fix, then continue the clean route |

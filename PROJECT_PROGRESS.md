...
- Current local verification: `npm.cmd run release:verify` passed 74 tests,
  ...
  `npm.cmd run native:sync` also passed after the final-enemy status-refresh
  fix.
...
| AI expert playtest and first usability fixes | Playtest complete; grouped copy, Town orientation, and readability follow-up are deployed; initial attack response is reproduced; full progression evidence remains open | `AI_EXPERT_PLAYTEST.md` records the fresh 1024x768 live journey, zero browser diagnostics, open-world observations, combat readability findings, modifier concern, and prioritized recommendations; onboarding/Town copy, minimap, lock banner, enemy silhouettes, and HP-bar treatment are live in the checkpoint shell; a later Mage run produced visible click damage and enemy defeats; local commit `3106820` covers the final-enemy stale-lock edge case | Continue the clean-profile C1 route and complete the touch/tablet lifecycle evidence; keep the local status-refresh fix grouped for the next deployment | Supported browser/device input, C1 combat evidence, and stable build | Yes for release usability; open-world suggestions are staged | Continue D1-8 evidence without changing combat calculations |

## Partially completed

| Feature or workstream | Current status | Evidence from the repository | Remaining work | Dependencies | Required for V1? | Recommended next action |
|---|---|---|---|---|---|---|
| Full V1 gameplay and progression | Runtime and automated route coverage exist; player-completed evidence is incomplete | `index.html` contains Town, four classes, combat, equipment, crafting, achievements, dungeon definitions, boss dispatches, and the D1-8 release guard; the latest local Mage QA run reached `Crypt Passage`, `Mystic Sanctum`, `Hidden Cache`, and `Burial Hall`, observed visible combat/rewards, and reproduced one hidden/offscreen Soul Wraith blocking room clear | Continue the bounded D1-8 path after the local `THREAT` locator and final-enemy stale-lock fixes; complete rooms, bosses, final portal, and intended session ending, recording failures and balance issues | Clean browser profile, supported input surface, and stable combat | Yes | Continue once the next supported-surface run can capture room-clear evidence |

...
visible local Gameplay Audio ON/OFF setting. Runtime `80a6562` is pushed with
source/mirror hash
`C76C4F8A11B6BE12ECF4BEDE19FB9A4D9526D20BCF1E559E2E2B3DC7516FB34A`; the
release gate is **147/147**, `qa:fast` clears all 11 released finales, and the
static/package/native checks pass. It is deployed at preview
`https://4aef14a3.code-quest-lab.pages.dev/` and the configured hostname. A
fresh live smoke verified the setting, reached first combat, defeated one
enemy, safely ended, cleaned up its disposable profile, and captured no
warnings/errors. The score remains **93%** / **19%** because clean-player
D1-D12 and physical/native-device acceptance are still open.
...
- Git state: the current deployed and locally tested runtime is `80a6562`
  (`Add optional gameplay audio feedback`) on `main`; D13+ remains gated from
...
- Current local verification: `npm.cmd run release:verify` passes **147/147**
...
- Current live verification: `npm.cmd run production:check -- https://code-quest-lab.gov8661682.com`
  ...
  hash is `C76C4F8A11B6BE12ECF4BEDE19FB9A4D9526D20BCF1E559E2E2B3DC7516FB34A`.
- Last website checkpoint: runtime `80a6562`, deployed and live-verified on
  2026-08-12 at `https://code-quest-lab.gov8661682.com/`; preview
  `https://4aef14a3.code-quest-lab.pages.dev/` also passed the production
  check and fresh settings/first-combat/safe-stop smoke.

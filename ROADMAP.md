...
Latest verified checkpoint (2026-08-07): the active source milestone `9c451af` completes a disposable Dungeon 2 QA route through the Fallen King and fixes queued level-up clarity; local release verification passes 81 tests, build/package audit, and native sync. The next checkpoint must push and deploy this tested source, then live-verify the configured domain. The active next priority after publication is Checkpoint 1 D4-8 progression, safe-stop session, and tablet evidence. Later milestones must repeat the same test, push, deploy, and live-verification sequence.

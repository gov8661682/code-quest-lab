...
- Latest deployed milestone runtime: `9da1d0e` (`Add local profile transfer and combat readability`); local hardening commits `5db6db5`, `3e33470`, and `8e165e1` are pushed to `origin/main` but not deployed because they are minor follow-ups. Prior stable runtime `32d83d0` and control-record commit `194bcc5` remain in history. `8e165e1` adds exact generated-route restoration on resume, corrects the first-combat room guard, and records the bounded off-viewport target finding.

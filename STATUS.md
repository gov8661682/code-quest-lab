...
- Latest deployed milestone runtime: `9da1d0e` (`Add local profile transfer and combat readability`); local hardening commits `5db6db5` and `3e33470` are pushed to `origin/main` but not deployed because they are minor follow-ups. Prior stable runtime `32d83d0` and control-record commit `194bcc5` remain in history. The current local follow-up adds exact generated-route restoration on resume, corrects the first-combat room guard, and records the bounded off-viewport target finding.
...
- Latest local verification: `npm.cmd run release:verify` passed with 65 Node tests, a 17-file build including deterministic `build-info.json`, and the static-package audit; `npm.cmd run native:sync` passed. The unreleased local shell and mirror share SHA-256 `E19FAB3F2F868554E00BA37C298790D534E990CCAA8D7DED4F375F572D517E31`; the deployed checkpoint remains `3A39EF4158EA494523FE04323D5D40BAA082E4C09F526A499707C3656EF139DA` until the next major milestone.

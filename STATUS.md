...
- Latest deployed milestone runtime: `9da1d0e` (`Add local profile transfer and combat readability`); hardening commit `5db6db5` is pushed to `origin/main` but not deployed because it is a minor follow-up. Prior stable runtime `32d83d0` and control-record commit `194bcc5` remain in history. The local follow-up adds verified session-transient UI cleanup and supported-surface combat evidence.
